...
def openNN(inpu,p):
    global input_data, net, output_data
    input_data = []
    output_data = []
    phase = []
        ...
        for i in range(len(tem)):
            if(len(tem[i]) != 0):
                ...
                for data in tem[i].split('\t'):
                    input_data[i].append(float(data))
    ...
    for i in range(len(input_data)):
        outp = net(torch.tensor(input_data[i]),phase[i])
        output_data.append(outp.detach().numpy())
...
def drawmodelAng():
    ...
    if(animate != -1):
        m = frame_num
        glTranslatef(output_data[m][48]/30.,output_data[m][49]/30.,output_data[m][50]/30.)
        glTranslatef(posi[0],posi[1],posi[2])
        glRotatef(-face,0.,1.,0.)
        #glRotatef(output_data[m][384], X[0], Y[0], Z[0]) 
        glRotatef(output_data[m][385], X[1], Y[1], Z[1]) 
        glRotatef(output_data[m][386], X[2], Y[2], Z[2]) 
    while(len(offset)>i):
        tem = np.array(offset[i])
        if(level_list[i-1] <= level_list[i]):
            draw2(tem)
            if(not(is_end_site(i))):
                ...
                if(animate != -1):
                    glRotatef(output_data[m][3*n + 384], X[3*n], Y[3*n], Z[3*n]) 
                    glRotatef(output_data[m][3*n + 385], X[3*n+1], Y[3*n+1], Z[3*n+1]) 
                    glRotatef(output_data[m][3*n + 386], X[3*n+2], Y[3*n+2], Z[3*n+2]) 
                    n = n + 1
        else:
            ii = level_list[i-1]-level_list[i]
            for t in range(ii):
...
class BVH(wx.Frame):
    ...
    def Timeover(self, event):
        global frames, frame_num,posi,output_data,face
        frame_num += 1
        face += output_data[frame_num][515]
        po0 = output_data[frame_num][216]/30.
        po2 = output_data[frame_num][218]/30.
        f = face/180.*np.pi
        posi[0] += np.cos(f)*po0 - np.sin(f)*po2
        posi[2] += np.sin(f)*po0 + np.cos(f)*po2
        if(frame_num >= frames - 120): 
            frame_num = 0
            posi = np.array([0.,0.,0.])
